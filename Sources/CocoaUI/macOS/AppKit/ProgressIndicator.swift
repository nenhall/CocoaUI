...
class ProgressIndicator: NSView {
    // cached data
    var particleGrad1: NSColor?
    var particleGrad2: NSColor?
    var particleGradient: NSGradient?
    var progressBarInnerShadow: NSShadow?
    var progressBarGradient: NSGradient?
    var progressBarLineGradient: NSGradient?
    var indeterminateGradientColor0: NSColor?
    var indeterminateGradientColor1: NSColor?
    var indeterminateGradientColor2: NSColor?
    var indeterminateGradientColor3: NSColor?
    var indeterminateGradient: NSGradient?
    ...
    var doubleValue = 0.0 {
        didSet {
            if doubleValue > doubleValue && !isIndeterminate {
                animator().internalDoubleValue = doubleValue
            } else {
    ...
    func progressBarRect() -> NSRect {
        ...
        return NSMakeRect(0, 0, round(CGFloat(scaledDoubleValue) * NSWidth(frame)), NSHeight(frame))
    }
...
//        NSDrawNinePartImage(dirtyRect, bezelTopLeftCorner, bezelTopEdgeFill, bezelTopRightCorner, bezelLeftEdgeFill, bezelCenterFill, bezelRightEdgeFill, bezelBottomLeftCorner, bezelBottomEdgeFill, bezelBottomRightCorner, .sourceOver, 1.0, false)
        ...
        let clipRect = NSMakeRect(0, 0, NSWidth(frame), NSHeight(progressBarRect()))
...
//        progressBarLineGradient?.draw(in: progressLinePath, angle: 90)
    ...
    func drawAnimationStep() {
        ...
        for i in 0..<Int(particlePitch) {
            ...
            let particleX = CGFloat((i * Int(particleDelta)) - currentAnimationStep)
            ...
            let gradientPoint = NSMakePoint(particleX + 17, NSHeight(frame) / 2.0)
    ...
    func drawIndeterminateAnimationStep() {
        ...
        if indeterminateGradient == nil {
            ...
            indeterminateGradient = NSGradient(colorsAndLocations:
        ...
        let particleDelta = CGFloat(kIndeterminateParticleWidth + kIndeterminateParticleSpacing - kIndeterminateParticleWidth / 2)
        
        for i in 0..<Int(particlePitch) {
            let particleX = CGFloat((i * Int(particleDelta)) + currentAnimationStep)
            
            let particlePath = NSBezierPath()
            particlePath.move(to: NSMakePoint(particleX - 10, NSHeight(frame)))
            particlePath.line(to: NSMakePoint(particleX + kIndeterminateParticleWidth / 2 - 30, NSHeight(frame)))
            ...
            particlePath.line(to: NSMakePoint(particleX - 30, NSHeight(frame)))
            particlePath.close()

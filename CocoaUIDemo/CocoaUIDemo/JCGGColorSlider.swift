...
@IBDesignable public class JCGGColorSlider: NSSlider {
    ...
    @IBInspectable public var barGradient: NSGradient = NSGradient.init(colors: [NSColor.systemBlue])!
    // Rainbow example
    ...
    public let bezelMargin: CGFloat = 10
    
    override public func draw(_ dirtyRect: NSRect) {
       
        if (self.isVertical) {
            if bounds.width - 4 > 24 {
                print("⚠️⚠️⚠️⚠️⚠️⚠️",self, "width 不能大于24")
            }
            // Bar styling
            ...
            // Bar styling
            if bounds.height - 8 > 19 {
                print("⚠️⚠️⚠️⚠️⚠️⚠️",self, "height 不能大于19")
            }
            barBackgroundColor.setFill()
            ...
            let kWidth = CGFloat((doubleValue - minValue) / maxValue) * innerRect.width
            if userInterfaceLayoutDirection == .leftToRight {
                if maxValue - minValue == 0 {
                    knobX = innerRect.minX
                } else {
                    knobX = innerRect.minX + kWidth
                }
            } else {
                if maxValue - minValue == 0 {
                    knobX = innerRect.maxX
                } else {
                    knobX = innerRect.maxX - kWidth
                }
            }
...
//            shadowPath.fill()
            ...
            let knobRect = NSRect(x: knobX - bounds.height * 0.5, y: 0, width: bounds.height, height: bounds.height).insetBy(dx: 2, dy: 2)

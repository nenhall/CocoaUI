...
open class Window: NSWindow {
    ...
    public private(set) lazy var backgroundView: TitlebarBackgroundView = {
    ...
    public private(set) lazy var effectView: NSVisualEffectView = {
    ...
    public private(set) lazy var leftToolBarBox: NSStackView = {
        let box = NSStackView()
    ...
    public private(set) lazy var rightToolBarBox: NSStackView = {
        let box = NSStackView()
        box.distribution = .fill
        box.orientation = .horizontal
        box.spacing = 20
    ...
    override open var toolbar: NSToolbar? {
        didSet {
            toolbar?.addObserver(self, forKeyPath: "items", options: [.new, .initial], context: nil)
        }
    }
    
    override public init(contentRect: NSRect, styleMask: NSWindow.StyleMask, backing: NSWindow.BackingStoreType, defer flag: Bool) {

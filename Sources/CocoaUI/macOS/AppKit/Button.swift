...
public class Button: NSButton, CALayerDelegate {
    ...
    /// 图标的位置
    override public var imagePosition: NSControl.ImagePosition {
        didSet {
    ...
    /// 图像缩放
    override public var imageScaling: NSImageScaling {
        didSet {
...
public extension Button {
    ...
    func resetColors() {
        for type in ColorableContentType.allCases {
...
private extension Button {
    ...
    func relayoutContent() {
        ...
        let (hasImage, hasTitle, isDraw) = (self.hasImage, self.hasTitle, isDrawUnderline)
        ...
        titleMax = NSMakeSize(min(titleMax.width, NSWidth(frame)), min(titleMax.height, NSHeight(frame)))
